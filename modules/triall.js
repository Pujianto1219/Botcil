...
async function triallssh(username, password, exp, iplimit, serverId) {
  ...
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
      ...
      const param = `:5888/triallssh?user=${username}&password=${password}&exp=${exp}&iplimit=${iplimit}&auth=${auth}`;
...
──────────────────────
*Save ACCOUNT TRIALL*: [Click Link](https://${sshData.domain}:81/ssh-${sshData.username}.txt)
──────────────────────
*IP Limit*: \`${sshData.ip_limit}\`
*Expired*: \`${sshData.expired}\`
──────────────────────
`;
              console.log('SSH account trialld successfully');
              return resolve(msg);
            } else {
...
async function triallvmess(username, exp, quota, limitip, serverId) {
  ...
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
      ...
      const domain = server.domain;
      const auth = server.auth;
      const param = `:5888/triallvmess?user=${username}&exp=${exp}&quota=${quota}&iplimit=${limitip}&auth=${auth}`;
      const url = `http://${domain}${param}`;
      axios.get(url)
        .then(response => {
          if (response.data.status === "success") {
...
\`\`\`
──────────────────────
*Save ACCOUNT TRIALL*: [Click Link](https://${vmessData.domain}:81/vmess-${vmessData.username}.txt)
──────────────────────
...
──────────────────────
`;
              console.log('VMess account trialld successfully');
              return resolve(msg);
            } else {
...
async function triallvless(username, exp, quota, limitip, serverId) {
  ...
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
      ...
      const domain = server.domain;
      const auth = server.auth;
      const param = `:5888/triallvless?user=${username}&exp=${exp}&quota=${quota}&iplimit=${limitip}&auth=${auth}`;
      const url = `http://${domain}${param}`;
      axios.get(url)
        .then(response => {
          if (response.data.status === "success") {
...
\`\`\`
──────────────────────
*Save ACCOUNT TRIALL*: [Click Link](https://${vlessData.domain}:81/vless-${vlessData.username}.txt)
──────────────────────
...
──────────────────────
`;
              console.log('VLESS account trialld successfully');
              return resolve(msg);
            } else {
...
async function trialltrojan(username, exp, quota, limitip, serverId) {
  ...
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
      ...
      const domain = server.domain;
      const auth = server.auth;
      const param = `:5888/trialltrojan?user=${username}&exp=${exp}&quota=${quota}&iplimit=${limitip}&auth=${auth}`;
      const url = `http://${domain}${param}`;
      axios.get(url)
        .then(response => {
          if (response.data.status === "success") {
...
\`\`\`
──────────────────────
*Save ACCOUNT TRIALL*: [Click Link](https://${trojanData.domain}:81/trojan-${trojanData.username}.txt)
──────────────────────
...
──────────────────────
`;
              console.log('Trojan account trialld successfully');
              return resolve(msg);
            } else {
...
async function triallshadowsocks(username, exp, quota, limitip, serverId) {
  ...
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
      ...
      const domain = server.domain;
      const auth = server.auth;
      const param = `:5888/triallshadowsocks?user=${username}&exp=${exp}&quota=${quota}&iplimit=${limitip}&auth=${auth}`;
      const url = `http://${domain}${param}`;
      axios.get(url)
        .then(response => {
          if (response.data.status === "success") {
...
\`\`\`
──────────────────────
*Save ACCOUNT TRIALL*: [Click Link](https://${shadowsocksData.domain}:81/ss-${shadowsocksData.username}.txt)
──────────────────────
...
──────────────────────
`;
              console.log('Shadowsocks account trialld successfully');
              return resolve(msg);
            } else {
...
module.exports = { triallssh, triallvmess, triallvless, trialltrojan, triallshadowsocks }; 
